...
class BoonsId extends StatefulWidget {
  final int id;

  const BoonsId({required this.id, Key? key}) : super(key: key);

  @override
  _BoonsIdState createState() => _BoonsIdState();
}

class _BoonsIdState extends State<BoonsId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Detalles bendiciones")),
      body: FutureBuilder(
        future: conectarDnDid('boons', '${widget.id}'),
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('boons', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('boons', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (BuildContext context) => PartidasIdMaster(id: widget.id, tabla: 'boons'),
                              ),
                            );
                          },
