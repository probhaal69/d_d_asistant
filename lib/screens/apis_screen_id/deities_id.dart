...
class DeitiesId extends StatefulWidget {
  final int id;

  const DeitiesId({required this.id, Key? key}) : super(key: key);

  @override
  _DeitiesIdState createState() => _DeitiesIdState();
}

class _DeitiesIdState extends State<DeitiesId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Detalles de deidad")),
      body: FutureBuilder(
        future: conectarDnDid('deities', '${widget.id}'),
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError) {
              ...
              List<CustomJsonContainer> listCustomContainer = [
                ...
                CustomJsonContainer(
                  titulo: "Categoria",
                  lista: '${registro['category']}',
                ),
                CustomJsonContainer(
                  titulo: "Dominios",
                  lista: registro['domains'],
                ),
                CustomJsonContainer(
                  titulo: "Juristiccion",
                  lista: '${registro['province']}',
                ),
                CustomJsonContainer(
                  titulo: "Simbolo",
                  lista: '${registro['symbol']}',
                ),
                CustomJsonContainer(
                  titulo: "Entradas",
                ...
                CustomJsonContainer(
                  titulo: "Todos sus nombres",
                  lista: registro['altNames'],
                ),
              ];
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('deities', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('deities', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (BuildContext context) => PartidasIdMaster(id: widget.id, tabla: 'deities'),
                              ),
                            );
                          },
