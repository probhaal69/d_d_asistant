...
class TablesId extends StatefulWidget {
  final int id;

  const TablesId({required this.id, Key? key}) : super(key: key);

  @override
  _TablesIdState createState() => _TablesIdState();
}

class _TablesIdState extends State<TablesId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Detalles de tabla")),
      body: FutureBuilder(
        future: conectarDnDid('tables', '${widget.id}'),
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('tables', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('tables', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
