...
class BackgroundsId extends StatefulWidget {
  final int id;

  const BackgroundsId({required this.id, Key? key}) : super(key: key);

  @override
  _BackgroundsIdState createState() => _BackgroundsIdState();
}

class _BackgroundsIdState extends State<BackgroundsId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Detalles de trasfondo")),
      body: FutureBuilder(
        future: conectarDnDid('backgrounds', '${widget.id}'),
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('backgrounds', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('backgrounds', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (BuildContext context) => PartidasIdMaster(id: widget.id, tabla: 'backgrounds'),
                              ),
                            );
                          },
