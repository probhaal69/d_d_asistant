...
class ItemsId extends StatefulWidget {
  final int id;

  const ItemsId({required this.id, Key? key}) : super(key: key);

  @override
  _ItemsIdState createState() => _ItemsIdState();
}

class _ItemsIdState extends State<ItemsId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: FutureBuilder(
        future: conectarDnDid('Items', '${widget.id}'),
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError) {
              ...
              List<CustomJsonContainer> listCustomContainer = [
                ...
                CustomJsonContainer(
                  titulo: "Sintonia",
                  lista: '${registro['attunement']}',
                ),
                CustomJsonContainer(
                  titulo: "Propiedades",
                  lista: '${registro['properties']}',
                ),
                CustomJsonContainer(
                  titulo: "Peso",
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('Items', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('Items', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (BuildContext context) => PartidasIdMaster(id: widget.id, tabla: 'Items'),
                              ),
                            );
                          },
