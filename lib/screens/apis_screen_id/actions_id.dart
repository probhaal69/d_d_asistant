...
class ActionId extends StatefulWidget {
  final int id;

  const ActionId({required this.id, Key? key}) : super(key: key);

  @override
  _ActionIdState createState() => _ActionIdState();
}

class _ActionIdState extends State<ActionId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Detalles de acción")),
      body: FutureBuilder(
        future: conectarDnDid('actions', '${widget.id}'),
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError) {
              ...
              List<CustomJsonContainer> listCustomContainer = [
                ...
                CustomJsonContainer(
                  titulo: "Accion relacionada",
                  lista: registro['seeAlsoAction'],
                ),
              ];
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('actions', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('actions', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (BuildContext context) => PartidasIdMaster(id: widget.id, tabla: 'actions'),
                              ),
                            );
                          },
