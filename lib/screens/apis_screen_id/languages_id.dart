...
class LanguagesId extends StatefulWidget {
  final int id;

  const LanguagesId({required this.id, Key? key}) : super(key: key);

  @override
  _LanguagesIdState createState() => _LanguagesIdState();
}

class _LanguagesIdState extends State<LanguagesId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Detalles del idioma")),
      body: FutureBuilder(
        future: conectarDnDid('languages', '${widget.id}'),
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError) {
              ...
              List<CustomJsonContainer> listCustomContainer = [
                ...
                CustomJsonContainer(
                  titulo: "habla el idioma",
                  lista: registro['typicalSpeakers'],
                ),
                CustomJsonContainer(
                  titulo: "Tipo",
                  lista: '${registro['type']}',
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('languages', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('languages', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (BuildContext context) => PartidasIdMaster(id: widget.id, tabla: 'languages'),
                              ),
                            );
                          },
