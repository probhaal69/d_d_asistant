...
class RacesId extends StatefulWidget {
  final int id;

  const RacesId({required this.id, Key? key}) : super(key: key);

  @override
  _RacesIdState createState() => _RacesIdState();
}

class _RacesIdState extends State<RacesId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Detalles de raza")),
      body: FutureBuilder(
        future: conectarDnDid('races', '${widget.id}'),
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError) {
              ...
              List<CustomJsonContainer> listCustomContainer = [
                ...
                CustomJsonContainer(
                  titulo: "Tipo de criatura",
                  lista: '${registro['creatureTypes']}',
                ),
                CustomJsonContainer(
                  titulo: "Tamaño",
                  lista: registro['size'],
                ...
                CustomJsonContainer(
                  titulo: "Condiciones inmunices",
                  lista: registro['conditionImmune'],
                ),
                CustomJsonContainer(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('races', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('races', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (BuildContext context) => PartidasIdMaster(id: widget.id, tabla: 'races'),
                              ),
                            );
                          },
