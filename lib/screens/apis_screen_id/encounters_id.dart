...
class EncountersId extends StatefulWidget {
  final int id;

  const EncountersId({required this.id, Key? key}) : super(key: key);

  @override
  _EncountersIdState createState() => _EncountersIdState();
}

class _EncountersIdState extends State<EncountersId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Detalles de encuentro")),
      body: FutureBuilder(
        future: conectarDnDid('encounters', '${widget.id}'),
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('encounters', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('encounters', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (BuildContext context) => PartidasIdMaster(id: widget.id, tabla: 'encounters'),
                              ),
                            );
                          },
