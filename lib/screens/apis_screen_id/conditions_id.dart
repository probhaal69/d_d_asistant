...
class ConditionsDiseasesId extends StatefulWidget {
  final int id;

  const ConditionsDiseasesId({required this.id, Key? key}) : super(key: key);

  @override
  _ConditionsDiseasesIdState createState() => _ConditionsDiseasesIdState();
}

class _ConditionsDiseasesIdState extends State<ConditionsDiseasesId> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: const Text("Detalles de condicion o enfermedad")),
      body: FutureBuilder(
        future: conectarDnDid('conditionsdiseases', '${widget.id}'),
          ...
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            if (snapshot.hasError) {
              ...
              List<CustomJsonContainer> listCustomContainer = [
                CustomJsonContainer(
                  titulo: "Condicion / enfermedad [${registro['id']}]",
                  lista: '${registro['name']}',
                ),
                CustomJsonContainer(
                  titulo: "Recurso",
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ElevatedButton(
                          onPressed: () async {
                            int success = await insertContenidoUser('conditionsdiseases', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                        ...
                        ElevatedButton(
                          ...
                          onPressed: () async {
                            int success = await removeContenidoUser('conditionsdiseases', idSesion!, widget.id);
                            switch (success) {
                              case 200:
                              case 201:
                                ScaffoldMessenger.of(context).showSnackBar(
                        ...
                        ElevatedButton(
                          onPressed: () {
                            Navigator.of(context).push(
                              MaterialPageRoute(
                                builder: (BuildContext context) => PartidasIdMaster(id: widget.id, tabla: 'conditionsdiseases'),
                              ),
                            );
                          },
