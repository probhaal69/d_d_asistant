...
class SubClassesDnD extends StatefulWidget {
  final dynamic filtroPartida;
  
  const SubClassesDnD({this.filtroPartida, Key? key}) : super(key: key);

  @override
  _SubClassesDnDState createState() => _SubClassesDnDState();
}

class _SubClassesDnDState extends State<SubClassesDnD> {
  String _searchTerm = '';
  ...
  Future<void> _loadUserId() async {
      ...
      setState(() {
        ...
        if (_mostrarListado == null) {
          _mostrarListado = _mostrarContenidoGlobal
              ? conectarDnDapi('subclass')
              : filtrarContenidoUser('subclass', idSesion!);
        }
    ...
    if (widget.filtroPartida != null) {
      _mostrarListado = filtrarContenidoPartida('subclass', widget.filtroPartida);
    }
  }
                            ...
                            ElevatedButton(
                              onPressed: () {
                                Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (BuildContext context) => const PartidasJugador(tabla: 'subclass'),
                                  ),
                                );
                              },
                                      ...
                                      child: FadeInAnimation(
                                        child: GestureDetector(
                                          onTap: () {
                                            Navigator.of(context).push(
                                              MaterialPageRoute(
                                                builder: (BuildContext context) => SubClassesId(id: registro['id'],)
                                              ),
                                            );
                                          },
                                          child: MostrarCampo(
