...
class NamesDnD extends StatefulWidget {
  final dynamic filtroPartida;
  
  const NamesDnD({this.filtroPartida, Key? key}) : super(key: key);

  @override
  _NamesDnDState createState() => _NamesDnDState();
}

class _NamesDnDState extends State<NamesDnD> {
  String _searchTerm = '';
  ...
  Future<void> _loadUserId() async {
      ...
      setState(() {
        ...
        if (_mostrarListado == null) {
          _mostrarListado = _mostrarContenidoGlobal
              ? conectarDnDapi('names')
              : filtrarContenidoUser('names', idSesion!);
        }
    ...
    if (widget.filtroPartida != null) {
      _mostrarListado = filtrarContenidoPartida('names', widget.filtroPartida);
    }
  }
                            ...
                            ElevatedButton(
                              onPressed: () {
                                Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (BuildContext context) => const PartidasJugador(tabla: 'names'),
                                  ),
                                );
                              },
                                      ...
                                      child: FadeInAnimation(
                                        child: GestureDetector(
                                          onTap: () {
                                            Navigator.of(context).push(
                                              MaterialPageRoute(
                                                builder: (BuildContext context) => NamesId(id: registro['id'],)
                                              ),
                                            );
                                          },
                                          child: MostrarCampo(
