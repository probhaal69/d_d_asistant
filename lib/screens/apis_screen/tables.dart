...
class TablesDnD extends StatefulWidget {
  final dynamic filtroPartida;
  
  const TablesDnD({this.filtroPartida, Key? key}) : super(key: key);

  @override
  _TablesDnDState createState() => _TablesDnDState();
}

class _TablesDnDState extends State<TablesDnD> {
  String _searchTerm = '';
  ...
  Future<void> _loadUserId() async {
      ...
      setState(() {
        ...
        if (_mostrarListado == null) {
          _mostrarListado = _mostrarContenidoGlobal
              ? conectarDnDapi('tables')
              : filtrarContenidoUser('tables', idSesion!);
        }
    ...
    if (widget.filtroPartida != null) {
      _mostrarListado = filtrarContenidoPartida('tables', widget.filtroPartida);
    }
  }
                            ...
                            ElevatedButton(
                              onPressed: () {
                                Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (BuildContext context) => const PartidasJugador(tabla: 'tables'),
                                  ),
                                );
                              },
                                      ...
                                      child: FadeInAnimation(
                                        child: GestureDetector(
                                          onTap: () {
                                            Navigator.of(context).push(
                                              MaterialPageRoute(
                                                builder: (BuildContext context) => TablesId(id: registro['id'],)
                                              ),
                                            );
                                          },
                                          child: MostrarCampo(
