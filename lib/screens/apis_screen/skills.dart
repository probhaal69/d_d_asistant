...
class SkillsDnD extends StatefulWidget {
  final dynamic filtroPartida;
  
  const SkillsDnD({this.filtroPartida, Key? key}) : super(key: key);

  @override
  _SkillsDnDState createState() => _SkillsDnDState();
}

class _SkillsDnDState extends State<SkillsDnD> {
  String _searchTerm = '';
  ...
  Future<void> _loadUserId() async {
      ...
      setState(() {
        ...
        if (_mostrarListado == null) {
          _mostrarListado = _mostrarContenidoGlobal
              ? conectarDnDapi('skills')
              : filtrarContenidoUser('skills', idSesion!);
        }
    ...
    if (widget.filtroPartida != null) {
      _mostrarListado = filtrarContenidoPartida('skills', widget.filtroPartida);
    }
  }
                            ...
                            ElevatedButton(
                              onPressed: () {
                                Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (BuildContext context) => const PartidasJugador(tabla: 'skills'),
                                  ),
                                );
                              },
                                      ...
                                      child: FadeInAnimation(
                                        child: GestureDetector(
                                          onTap: () {
                                            Navigator.of(context).push(
                                              MaterialPageRoute(
                                                builder: (BuildContext context) => SkillsId(id: registro['id'],)
                                              ),
                                            );
                                          },
                                          child: MostrarCampo(
