...
class EncountersDnD extends StatefulWidget {
  final dynamic filtroPartida;
  
  const EncountersDnD({this.filtroPartida, Key? key}) : super(key: key);

  @override
  _EncountersDnDState createState() => _EncountersDnDState();
}

class _EncountersDnDState extends State<EncountersDnD> {
  String _searchTerm = '';
  ...
  Future<void> _loadUserId() async {
      ...
      setState(() {
        ...
        if (_mostrarListado == null) {
          _mostrarListado = _mostrarContenidoGlobal
              ? conectarDnDapi('encounters')
              : filtrarContenidoUser('encounters', idSesion!);
        }
    ...
    if (widget.filtroPartida != null) {
      _mostrarListado = filtrarContenidoPartida('encounters', widget.filtroPartida);
    }
  }
                            ...
                            ElevatedButton(
                              onPressed: () {
                                Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (BuildContext context) => const PartidasJugador(tabla: 'encounters'),
                                  ),
                                );
                              },
                                      ...
                                      child: FadeInAnimation(
                                        child: GestureDetector(
                                          onTap: () {
                                            Navigator.of(context).push(
                                              MaterialPageRoute(
                                                builder: (BuildContext context) => EncountersId(id: registro['id'],)
                                              ),
                                            );
                                          },
                                          child: MostrarCampo(
