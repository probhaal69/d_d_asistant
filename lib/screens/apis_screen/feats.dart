...
class FeatsDnD extends StatefulWidget {
  final dynamic filtroPartida;
  
  const FeatsDnD({this.filtroPartida, Key? key}) : super(key: key);

  @override
  _FeatsDnDState createState() => _FeatsDnDState();
}

class _FeatsDnDState extends State<FeatsDnD> {
  String _searchTerm = '';
  ...
  Future<void> _loadUserId() async {
      ...
      setState(() {
        ...
        if (_mostrarListado == null) {
          _mostrarListado = _mostrarContenidoGlobal
              ? conectarDnDapi('feats')
              : filtrarContenidoUser('feats', idSesion!);
        }
    ...
    if (widget.filtroPartida != null) {
      _mostrarListado = filtrarContenidoPartida('feats', widget.filtroPartida);
    }
  }
                            ...
                            ElevatedButton(
                              onPressed: () {
                                Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (BuildContext context) => const PartidasJugador(tabla: 'feats'),
                                  ),
                                );
                              },
                                      ...
                                      child: FadeInAnimation(
                                        child: GestureDetector(
                                          onTap: () {
                                            Navigator.of(context).push(
                                              MaterialPageRoute(
                                                builder: (BuildContext context) => FeatsId(id: registro['id'],)
                                              ),
                                            );
                                          },
                                          child: MostrarCampo(
