...
class RacesDnD extends StatefulWidget {
  final dynamic filtroPartida;
  
  const RacesDnD({this.filtroPartida, Key? key}) : super(key: key);

  @override
  _RacesDnDState createState() => _RacesDnDState();
}

class _RacesDnDState extends State<RacesDnD> {
  String _searchTerm = '';
  ...
  Future<void> _loadUserId() async {
      ...
      setState(() {
        ...
        if (_mostrarListado == null) {
          _mostrarListado = _mostrarContenidoGlobal
              ? conectarDnDapi('races')
              : filtrarContenidoUser('races', idSesion!);
        }
    ...
    if (widget.filtroPartida != null) {
      _mostrarListado = filtrarContenidoPartida('races', widget.filtroPartida);
    }
  }
                            ...
                            ElevatedButton(
                              onPressed: () {
                                Navigator.of(context).push(
                                  MaterialPageRoute(
                                    builder: (BuildContext context) => const PartidasJugador(tabla: 'races'),
                                  ),
                                );
                              },
                                      ...
                                      child: FadeInAnimation(
                                        child: GestureDetector(
                                          onTap: () {
                                            Navigator.of(context).push(
                                              MaterialPageRoute(
                                                builder: (BuildContext context) => RacesId(id: registro['id'],)
                                              ),
                                            );
                                          },
                                          child: MostrarCampo(
